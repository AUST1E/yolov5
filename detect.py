...
def detect(opt,save_img=False):
    ...
    connect = pymssql.connect('DESKTOP-5S2RBLU\\SQL2014', 'sa', 'z458176711', 'WHSJ_Computer_Board')  # 建立连接
    if connect:
        print("连接成功!")
    ...
    cursor.execute(
        "create table Defects_nums{0} (Defects_Dno char(20),Defects_Dname char(50),Defects_Dnum INT ) insert "
        "into Defects_nums{0} values ('A01','风扇螺丝松动','0'),('A02','缺少风扇螺丝','0'),('A03','主板螺丝型号不正确','0'),"
        "('A04','缺少主板螺丝','0'),('A05','风扇接线不正确','0')".format(otherStyleTime))  # 执行sql语句
    connect.commit()  # 提交
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                     ...
                     if names[int(c)] == '1 Loose fan screws' :
                         cursor.execute(
                             "update Defects_nums{0} set Defects_Dnum = Defects_Dnum+ {1} WHERE Defects_Dno = 'A01'".format(
                                 otherStyleTime, n))  # 执行sql语句
                         connect.commit()  # 提交
                     elif names[int(c)] == '3 Missing fan screws':
                         cursor.execute(
                             "update Defects_nums{0} set Defects_Dnum = Defects_Dnum+ {1} WHERE Defects_Dno = 'A02'".format(
                                 otherStyleTime, n))  # 执行sql语句
                         connect.commit()  # 提交
                     elif names[int(c)] == '6 board screw model error':
                         cursor.execute(
                             "update Defects_nums{0} set Defects_Dnum = Defects_Dnum+ {1} WHERE Defects_Dno = 'A03'".format(
                                 otherStyleTime, n))  # 执行sql语句
                         connect.commit()  # 提交
                     elif names[int(c)] == '7 Missing board screws':
                         cursor.execute(
                             "update Defects_nums{0} set Defects_Dnum = Defects_Dnum+ {1} WHERE Defects_Dno = 'A04'".format(
                                 otherStyleTime, n))  # 执行sql语句
                         connect.commit()  # 提交
                     elif names[int(c)] == '9 Incorrect fan wiring':
                         cursor.execute(
                             "update Defects_nums{0} set Defects_Dnum = Defects_Dnum+ {1} WHERE Defects_Dno = 'A05'".format(
                                 otherStyleTime, n))  # 执行sql语句
                         connect.commit()  # 提交
                # Write results
    ...
    print(y)
    x = ('风扇螺丝松动', '缺少风扇螺丝', '主板螺丝型号不正确', '缺少主板螺丝', '风扇接线不正确')
    plt.bar(x, y, 0.4)
    plt.ylim(0, 20)
    plt.title("主板质量检测报表")
    plt.xlabel("缺陷名称")
